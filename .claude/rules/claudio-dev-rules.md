...
- **分叉**：杀后台重进恢复的**权威源**——远端 agent 用 bridge history 回填（官方语义，bridge 是唯一真相）；本地 agent 用本地 DB + uiSequence 快照（本地 DB 是唯一真相）
- **快照定位**：uiSequence 快照退回本地 agent 专属。远端在 A 方案落地前用修复后的快照（a9c67f4，FINAL FLUSH 后重拍）兜底，落地后退役或降为离线兜底
