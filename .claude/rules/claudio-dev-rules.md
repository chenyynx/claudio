...
## 🔴🔴 最高规则：本地/远端 agent 死隔离（2026-09-09 pp 定，凌驾一切开发任务）

**任何开发任务动手前必须先制定隔离方案**，四问必答：
1. **改了这个会不会影响另一端？** 本地 agent（进程内 Swift，成熟稳定系统）与远端 agent（bridge/WS/Claude Code 链路）双向零污染。
2. **共享文件的改动有没有显式 gate？** isRemoteAgent / source 结构化判断 / RemoteXxx 类型判断；无 gate 的共享路径改动 = 违规，改完必须证明另一端行为零变化。
3. **官方有没有等价物？** 给本地 agent 加任何机制前先查 OpenMinis 官方——官方没有的机制不得私自发明；官方本来就稳定，偏离官方的就是出问题的。
4. **另一端的回归项写了没有？** 每批改动必含本地回归 + 远端回归，缺一不可。

**架构铁律（核心原则，2026-09-09 pp 定）**：
- **远端 Agent 是扩展独立大模块，不得反向侵入或污染本地 Agent**
- **新增功能优先通过独立模块、Provider、Adapter、Bridge 或明确的抽象接口实现**，而不是为了复用、方便或统一架构而修改本地 Agent 内部逻辑
- **共享只能发生在明确设计的公共接口、数据模型或必要的 Shared 层**，不得直接耦合双方内部状态、状态机、Session 生命周期、工具调用流程和运行逻辑

**不怕麻烦，就怕意外**：隔离成本永远小于事故成本。最高隔离，机械执行，无例外。
反面教材：uiSequence 快照机制（99e2828d 引入，本地/远端共用路径全量拍快照 → thinking 块重复渲染事故 + 8 天潜伏）——2026-09-09 已整套移除对齐官方。
...
### 恢复层分叉原则（2026-09-01 pp 定，快照丢正文事故后）

- **继续共享**：渲染/UI（工具卡片、markdown、块）、流式消费（SSEStream 状态机）——保体验统一、白拿基建
- **分叉**：杀后台重进恢复的**权威源**——远端 agent 用 bridge history 回填（官方语义，bridge 是唯一真相）；本地 agent 用本地 DB parts + reasoning_content（官方路径，本地 DB 是唯一真相。~~uiSequence 快照~~ 已于 2026-09-09 整套移除，见下条）
- **快照定位（2026-09-09 更新）**：uiSequence 快照机制**已整套退役移除**（引入时的全量快照语义导致 DB 行 k 携带前 k 轮全部块 → 重进重建 (N-k+1) 次重复渲染；官方本无此机制）。本地/远端恢复统一官方路径：本地 = parts + reasoning_content 合并单块头部；远端 = bridge history 回填。**任何"给本地 agent 加恢复/渲染新机制"的想法，先过顶部最高规则四问**
- **A 方案提级**：远端恢复切 bridge history 从「M2/M3 后评估」提级到「**M2 之前做**」——M2 工具卡片渲染直接按官方 content 数组渲染设计，避免快照版工具结果恢复做一遍再重写
- 双事实源禁令：同一通道的恢复只认一个权威源，快照与 parts 内容必须严格同源（a9c67f4 教训：快照早拍 + 优先级压过 parts = 正文丢失）
- 壳层（AIChatViewModel 持久化/UI 同步）是共享改造区，任何改动必须过三-C 壳层回归（验证点 R），本地/远端双通道都测
...
3. **模块化**（按"职责/内聚"分文件，不是按行数）。3 不变量 + 5 触发条件 + 软警告：①不变量：一个文件 = 一个公开类型 + 私有 helper；一个文件 = 一个"改变的理由"（SRP）；Cohesion 至少 Communicational。②触发条件（满足任一即拆）：≥2 公开类型 / ≥2 独立 @State/@Published 状态域 / type-check 超时 / 改一处想复用要 copy-paste / 单测要 import 整个文件但只测一个 20 行 helper。③软警告：100 行自查 / 200 行触发自检 / 500 行必须拆 / 函数体 > 50 行必须拆。禁止死代码/内联重复/中间 import（详见 [[../../SecondBrain/Agent-长期记忆/开发规范]]）
4. **最小改动。** 只改本次任务必需的文件和行，不顺手"统一风格"
5. **GPL 意识。** App 内所有新代码都要开源——注释/命名按开源标准写，不写 proprietary 标记
6. **协议照 K9i-0/ccpocket 公开文档（wire 层）。** 不自创消息格式；看不懂协议先查文档（stack.md）+ 官方客户端源码，不猜。⚠️ heypandax/cc-pocket 是同名不同项目，已排除（见 preflight 决策记录 0）。**展示层/UI 交互不对齐 ccpocket，按 pp 拍板设计（2026-09-09 修订，见 C-0）。**
7. **Bug 入库。** 修复完成 → 查 Bug经验库 → 判定是否入库 → 入库后才告知"修好了"
8. **会话状态。** 会话结束时更新本仓库会话状态（文件列表/行数/已知限制/下一步）
9. **UI 改动先列问题清单。** 对齐行为规范：UI/动画/布局改动不许边改边试

## 三-D、Doris（手机端 Minis agent）协作协议（2026-09-01 pp 定）

Doris 与服务器 CC 并发开发 claudio 时的追加约定（Doris 全部遵守；CC 侧按此识别 Doris 的动作）：

1. **认领/归还格式**（写在状态文件断点区，动代码前认领、干完归还）：
   - `[认领] Doris | 时间 | 动哪块 | 预期文件`
   - `[归还] Doris | 时间 | commit号 | 断点说明`
...
**🟢 2026-09-09 pp 修订：ccpocket 官方从"唯一基准"降级为"参考实现之一"。** 不再要求全部功能照抄 ccpocket——Claudio 是自己的产品，交互按 Claudio 自己的产品逻辑设计（pp 拍板优先）。ccpocket 源码仍是**协议层**的权威参照（wire 格式、事件语义、连接行为照它对齐，这部分不变），但**展示层/UI 交互**（图片渲染位置、卡片展开逻辑、消息布局等）不再强制对齐，pp 要什么就做什么。看官方代码改成按需查证（协议行为存疑时才去翻），不是默认动作。

**远端通道的功能开发流程**：协议层照 ccpocket（不变）；产品/交互层 → 先问 pp 要什么效果 → 复述理解 → 清单拍板 → 实现。与官方不一致的地方在 C-3 表"设计差异"列注明理由即可，不算违规。

### C-1. 翻车根因三连（2026-09-01 pp 点名，防再犯）

> 2026-09-09 起 C-1 的"读官方"要求**只适用于协议/连接层行为**（重连、resume、队列、事件消费）；展示层功能不受此限。

1. **子代理审计清单 ≠ 自己读官方代码**：前几轮派子代理审计官方源码，输出"功能有无"清单（有重连 ✅ 有 resume ✅）→ 漏了"重连时发不发 resume"这个行为细节 → 进程堆积修了几轮没根治。**涉及协议行为的排查必须亲自打开官方源码对应文件/函数读，不依赖子代理清单。**
2. **"目标症状消失" ≠ "修完了"**：每轮修完只验证目标症状，过了就宣布完成。正确完成标准 = C-4 对照表全部填完且差异分类完毕。
3. **"照协议写"被理解成"照消息格式写"**：公开协议文档（stack.md）只讲 wire 格式，**协议行为语义在官方客户端代码里**。协议行为问题先找官方实现，不靠协议文档猜。

### C-2. 动手前置（每项远端通道功能开工前必做，逐项勾完才准动代码）

**开工 checklist（每个远端通道功能/修复必须全勾）：**

- [ ] **协议层行为**（连接/重连/resume/队列/事件消费）：官方源码定位 `~/refs/ccpocket-main` 找到对应实现文件+函数，通读（不靠子代理转述）；写 `// [Fix] 对齐官方 xxx.dart:yyy — <行为一句话>`
- [ ] **展示层/UI 功能**：跳过官方源码定位，直接按 pp 拍板的产品效果设计；与官方不一致属正常（设计差异，回写 C-3 注明即可）
...
- [ ] 给 pp 复述理解 + 改动清单（铁律 1），确认后才写
- [ ] 检查 C-3 基准表：协议层行为已对齐的不得偏离；展示层不适用此约束
...
```
基准行为（协议层=官方源码位置 / 展示层=pp 拍板的产品效果）：____
我们现状：____
差异：____
差异分类：□协议行为（必须对齐） □设计差异（pp 拍板/写明理由） □低优先级（记待办）
验证方式：____
出包验证点：____
```

- **协议行为**对齐的差异 → 本轮修掉，**并回写进 C-3 行为基准表**（新行：官方基准 + 我们的实现 + 对齐 commit）——不回写 = 没闭环
- **设计差异**（pp 拍板的产品效果，如 stopped 后从会话列表移除=会丢本地历史）→ 写明理由
- **低优先级**（如 baseSeq 多端冲突检测）→ 记入状态文件待办 + C-5
- 表没填完 = 修复没完成，不许说"修好了"

**出包验证模板**（发给 pp 的验证点逐条对应产品效果）：

```
验证点 1：____（对应产品效果：____）
通过标准：____
验证点 2：____（对应产品效果：____）
通过标准：____
```

**壳层回归（任何 AIChatViewModel 持久化/UI 同步层改动必做，2026-09-01 pp 定）**：

```
验证点 R：长任务（thinking + 工具×N + 多段正文）完整跑完 → 杀后台 → 重进
通过标准：正文每段、thinking 分段、工具块与杀后台前渲染逐块一致
（对照官方行为：渲染 claude content 数组原始顺序，官方无本地快照概念）
```
