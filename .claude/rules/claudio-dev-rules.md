...
- [ ] 官方源码定位：在 `~/refs/ccpocket-main（原 /tmp 路径留符号链接，重启后请用 ~/refs）` 找到对应实现文件+函数（`bridge_service.dart` / `claude_session_screen.dart` / `chat_session_cubit.dart` / `codex_session/` / `session_list/`）
...
出包验证点（对应官方行为，如"切后台 10 次进程数不变"= 官方"重连不 spawn"）：____
```

- **必须对齐**的差异 → 本轮修掉，**并回写进 C-3 行为基准表**（新行：官方基准 + 我们的实现 + 对齐 commit）——不回写 = 没闭环
- **设计差异**（如 stopped 后从会话列表移除=会丢本地历史）→ 写明理由，不照抄
- **低优先级**（如 baseSeq 多端冲突检测）→ 记入状态文件待办 + C-5
- 表没填完 = 修复没完成，不许说"修好了"

**出包验证模板**（发给 pp 的验证点必须逐条对应官方行为）：

```
验证点 1：____（对应官方行为：____）
通过标准：____
验证点 2：____（对应官方行为：____）
通过标准：____
```
