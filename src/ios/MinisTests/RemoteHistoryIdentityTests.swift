...
final class RemoteHistoryIdentityTests: XCTestCase {
    ...
    func test_claim_byClientMessageId_evenWhenTextDiffers() {
        ...
        XCTAssertEqual(index.claimOwner(for: raw), "UUID-A")
    }
    ...
    func test_claim_imageOnlyRow_byClientMessageId() {
        ...
        var index = RemoteHistoryOwnerIndex(dbRows: [local], excluding: [])
        XCTAssertEqual(index.claimOwner(for: raw), "UUID-IMG",
    ...
    func test_claim_imageOnlyRow_withoutClientId_returnsNil() {
        ...
        var index = RemoteHistoryOwnerIndex(dbRows: [local], excluding: [])
        XCTAssertNil(index.claimOwner(for: raw), "无身份的纯图片行配不上（不猜）")
    }
    ...
    func test_claim_clientIdTakesPrecedenceOverText() {
        ...
        XCTAssertEqual(index.claimOwner(for: rawWithId), "UUID-A")

        let rawWithoutId = row(id: "bridge-ns-seg-2", parts: [.text("hi")])
        XCTAssertEqual(index.claimOwner(for: rawWithoutId), "UUID-B",
    ...
    func test_claim_textFallback_pairsDuplicatesByOccurrence() {
        ...
        var index = RemoteHistoryOwnerIndex(dbRows: [a, b], excluding: [])
        XCTAssertEqual(index.claimOwner(for: row(id: "bridge-ns-seg-1", parts: [.text("在吗")])), "UUID-A")
        XCTAssertEqual(index.claimOwner(for: row(id: "bridge-ns-seg-3", parts: [.text("在吗")])), "UUID-B")
    }
    ...
    func test_claim_oneLocalRowClaimedOnce() {
        ...
        var index = RemoteHistoryOwnerIndex(dbRows: [local], excluding: [])
        XCTAssertEqual(index.claimOwner(for: row(id: "bridge-ns-seg-1", parts: [.text("在吗")])), "UUID-A")
        XCTAssertNil(index.claimOwner(for: row(id: "bridge-ns-seg-2", parts: [.text("在吗")])),
                     "一个本地行只能顶替一个位置——第二个回放行必须正常插入")
    }
    ...
    func test_claim_excludesRowsScheduledForDeletion() {
        ...
        XCTAssertNil(index.claimOwner(for: row(id: "past-1", parts: [.text("在吗")])))
    }
    ...
    func test_claim_toolResultByToolUseId() {
        ...
        XCTAssertEqual(index.claimOwner(for: raw), "UUID-TR")
    }
    ...
    func test_claim_assistantRowsNeverClaim() {
        ...
        var index = RemoteHistoryOwnerIndex(dbRows: [local], excluding: [])
        XCTAssertNil(index.claimOwner(for: row(id: "bridge-ns-seg-1", role: .assistant, parts: [.text("hi")])),
    ...
    func test_claim_emptyTextRowIsNotATextMatch() {
        ...
        var index = RemoteHistoryOwnerIndex(dbRows: [local], excluding: [])
        XCTAssertNil(index.claimOwner(for: row(id: "bridge-ns-seg-1", parts: [.text("")])),
