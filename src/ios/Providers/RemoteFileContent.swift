...
struct RemoteFileContentFetcher: Sendable {
    ...
    static func absoluteMediaURL(relative: String, httpBaseURL: URL?) -> URL? {
        ...
        // [Claudio 2026-09-06] 调 RemoteFileContent.composeBridgeAbsoluteURL
        // — 共享 helper 在 enum RemoteFileContent 里(static member),
        // 不用全局函数避免 RemoteFileUpload/Download 重复定义
        // "invalid redeclaration"。
        return RemoteFileContent.composeBridgeAbsoluteURL(relative: relative, base: httpBaseURL)
    }
