...
/// 深海氛围背景（移植自 dsh-mobile HarnessAnimatedBackground，已去品牌化）
...
struct OceanAnimatedBackground: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 30.0, paused: pausesAnimation)) { timeline in
            ...
            ZStack {
                OceanFluidLayer(time: elapsed)
                OceanTechnicalGrid()
                OceanParticleField(time: elapsed, isStatic: reduceMotion)
                    .blendMode(.screen)

                LinearGradient(
                    colors: [.clear, OceanColor.navy.opacity(0.12), Color.black.opacity(0.58)],
                    startPoint: .top,
                    endPoint: .bottom
                )
            }
        }
        .background(OceanColor.navy)
        .ignoresSafeArea()
...
private struct OceanFluidLayer: View {
...
private struct OceanTechnicalGrid: View {
    var body: some View {
...
private struct OceanParticleField: View {
    let time: TimeInterval
    let isStatic: Bool

    private let particles = GhostParticleMask.particles
...
private struct GhostParticle: Sendable {
    let x: CGFloat
    let y: CGFloat
...
private enum GhostParticleMask {
    static let particles: [GhostParticle] = makeParticles()

    private static func makeParticles(sampleSize: Int = 60) -> [GhostParticle] {
        guard let image = UIImage(named: "HeroGhostMask") else { return [] }
